...
export const REQUEST_POSTS = 'REQUEST_ModelPagePOSTS';
export const RECEIVE_POSTS = 'RECEIVE_ModelPagePOSTS';
export const INVALIDATE_SUBREDDIT = 'INVALIDATE_ModelPageSUBREDDIT';


function requestPosts(api) {
  return {
    type: REQUEST_POSTS,
    payload: 'Requesting Posts in modelPage',
  };
}
...
export function invalidateSubreddit(api) {
  return {
    ...
    payload: 'Invalidation of posts in modelPage',
  };
}

export function fetchPosts(api, type, modelname) {
...
// });
    ...
    return Prismic.api(apiEndpoint)
      .then(
        (api) => api.getByUID(`${type}`, `${modelname}`)
